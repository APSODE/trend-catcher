...
class HashtagSearchService:
    ...
    #해시태그별 뉴스 pk 반환
    async def search_all(self, hashtags: list[str]) -> dict[str, list[str]]:
        #빈 리스트 방어
        if not hashtags:
            return {}
    ...
    async def _collect_crawled_ids(self, expanded_list: list[HashtagModel], keyword_pk_list: list[set[int]]) -> dict[str, list[str]]:
        ...
        for expanded, keyword_pks in zip(expanded_list, keyword_pk_list):
            ...
            news_list = await self._news_analysis_repository.find_by_pks(news_pks)
            result[expanded.hashtag] = [news.crawled_id for news in news_list]
        return result
