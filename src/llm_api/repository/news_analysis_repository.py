...
class NewsAnalysisRepository:
    #새 뉴스 추가
    def save(self, session: Session, news_analysis: NewsAnalysisModel) -> NewsAnalysisModel:
        session.add(news_analysis)
        session.flush()
        return news_analysis

    #분석여부 확인
    def is_exist(self, session: Session, target_id: str)-> bool:
    ...
    def get_main_news(self, session: Session, cutoff_score: float, limit: int) -> list[NewsAnalysisModel]:
